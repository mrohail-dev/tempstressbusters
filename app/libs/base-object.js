...
function canFav(object) {
  if (
    object.type == strings.OBJECT_TYPE_CALLBACK ||
...
function canReplySms(object) {
  return Boolean(
    (object.type == strings.OBJECT_TYPE_GROUP ||
      object.type == strings.OBJECT_TYPE_LINK) &&
      !!object.phone &&
...
function canLinkToMap(object) {
  ...
  if (object.type == strings.OBJECT_TYPE_LINK) {
    return Boolean(
      object.address && object.address != null && object.address != '',
    );
  }
...
function canParticipate(object) {
  return Boolean(
    object.type === strings.OBJECT_TYPE_EVENT && object.can_signup,
  );
}
...
function isPaid(object) {
  if (object.type == strings.OBJECT_TYPE_AUDIO) {
    return (
      object.id !== 'hccF8tQMJy' && // Quick Calm
    ...
    if (object.related_screen) {
      const route = Object.keys(routes).find(routeFunc => {
        ...
        return Boolean(
          object.related_screen == route.relatedName && !route.free,
        );
      });
      if (route) {
        return true;
